...
def main():
    ...
    sen1 = 1*np.sin(2*np.pi*freq[num][0]*t)
    sen2 = 1*np.sin(2*np.pi*freq[num][1]*t)
    ...
    # Plota o gráfico no domínio da frequência da soma das frequências somadas
    sinal = signalMeu()
    ...
    # Plota o gráfico no domínio do tempo da soma das frequências somadas
    plt.figure()
    plt.plot(t[:1000], tone[:1000])
    plt.title('Domínio do Tempo da Soma')
    plt.xlabel('Tempo [s]')
    plt.ylabel('Amplitude')
    plt.grid(True)
    plt.show()
